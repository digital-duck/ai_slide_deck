...
async def generate_pdf_playwright(slides_dir: Path, slides: List[Dict], output_file: Path, page_title: str) -> None:
    ...
    async with async_playwright() as p:
        ...
        if pdf_pages:
            # For simplicity, we'll save the first page as example
            # In a real implementation, you'd use a PDF library like PyPDF2 to merge
            with open(output_file, 'wb') as f:
                f.write(pdf_pages[0])  # This saves only the first page as example
            
            click.echo(f"✅ PDF generated: {output_file}")
        else:
            click.echo("❌ No valid slides found for PDF generation")

def generate_pdf_weasyprint(slides_dir: Path, slides: List[Dict], output_file: Path, page_title: str) -> None:
    """Generate PDF using WeasyPrint (alternative method)"""
    if not WEASYPRINT_AVAILABLE:
        raise ImportError("WeasyPrint not available. Install with: pip install weasyprint")
    
    click.echo("🖨️  Generating PDF with WeasyPrint...")
    ...
    for i, slide in enumerate(slides):
        ...
        body_match = re.search(r'<body[^>]*>(.*?)</body>', slide_content, re.DOTALL)
        if body_match:
            body_content = body_match.group(1)
            page_break = "page-break" if i > 0 else ""
            combined_html += f'<div class="{page_break}">{body_content}</div>\n'
    
    combined_html += "</body></html>"
    
    # Generate PDF
    try:
        weasyprint.HTML(string=combined_html).write_pdf(str(output_file))
        click.echo(f"✅ PDF generated: {output_file}")
    except Exception as e:
        click.echo(f"❌ WeasyPrint error: {e}")
        raise
...
@click.option(
    '--method',
    '-m',
    type=click.Choice(['playwright', 'weasyprint']),
    default='playwright',
